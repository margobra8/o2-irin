...
double CIriFitnessFunction::GetFitness()
{
	...
	double fit = (m_fComputedFitness / (double)m_unNumberOfSteps) * (1 - ((double)(fmin(coll, 20.0) / 20.0)));
...
void CIriFitnessFunction::SimulationStep(unsigned int n_simulation_step, double f_time, double f_step_interval)
{
	...
	//*  START FITNESS 1 -- batteries stdev weighting

	// double LOW_BATT_TH = 0.2;
	// double bb = blueBattery[0];
	// double b = battery[0];
	// double battArr[2] = {bb, b};

	// double penalization = 1.0;

	// // varianza del valor de la bateria (cuanto mas alejados esten los valores, mayor sera la varianza)

	// // penalization
	// if (bb < LOW_BATT_TH || b < LOW_BATT_TH)
	// {
	...
	// for (int i = 0; i < 2; i++)
	// {
	// 	temp += (battArr[i] - mean) * (battArr[i] - mean);
	// }

	// double battVariance = sqrt(temp / (2 - 1));

	// // que valor de que bateria es el minimo y multiplicar todo por el valor maximo de los sensores de luz de la bateria que corresponda

	// double lightGoalEval;

	// if (bb < b)
	// {
	// 	lightGoalEval = maxBlueLightSensorEval;
	// }
	// else
	// {
	// 	lightGoalEval = maxLightSensorEval;
	// }

	// double fitness = (1 - battVariance) * penalization * lightGoalEval + (maxSpeedEval/2 * sameDirectionEval);

    //* END FITNESS 1

	//* START FITNESS 2 -- Schmidt trigger / astable osc

	double BB_H_TH = 0.6;
	double BB_L_TH = 0.1;
	double bb = blueBattery[0];
	double b = battery[0];
	double penalization1 = 1.0;
	double penalization2 = 1.0;

	int res;
	// guardar variable current state nueva en la clase
	int upperLimit = (b > BB_H_TH);
	int lowerLimit = (b < BB_L_TH);

	res = lowerLimit || (curState && !upperLimit);

	curState = res;

	if (res)
	{
		//Aquí estamos en la situación en la que la batería se debe ir a cargar
		penalization2 = 0;
	}
	else
	{
		// Aquí estamos en la situación en la que debe evitar cargarse e ir hacia la luz azul
		penalization1 = 0;
	}
	double fitness = ((b * penalization1) + (((blueLightS0 + blueLightS7) / 2) * penalization2)) + 0.5 * (maxSpeedEval * sameDirectionEval);
